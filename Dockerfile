...
RUN cd /tmp/assessment_src/src/ && tar -xjf php-5.2.17.tar.bz2

# Change to the extracted directory
WORKDIR /tmp/assessment_src/src/php-5.2.17/

# Apply the patch
RUN patch -p0 < /tmp/assessment_src/src/txtbgxGXAvz4N.txt
...
# Change to the extracted directory
WORKDIR /tmp/assessment_src/src/

# Run the script without sudo
RUN chmod +x mysql5.7-installation.sh && bash -x mysql5.7-installation.sh
